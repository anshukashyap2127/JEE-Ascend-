...
let isSyncingFromCloud = false; // Flag to prevent infinite re-push loops
      ...
      if (isCapacitorApp) {
        ...
        db.enablePersistence({ synchronizeTabs: true }).catch((err) => {
          if (err.code === 'failed-precondition') {
            console.warn("Multiple tabs open; persistence in primary tab.");
          }
        });
      }
    }
...
// BULLETPROOF STATE MERGER (No crashes, preserves all data)
function mergeStates(local, cloud) {
  if (!cloud || typeof cloud !== 'object') return local || state;
  if (!local || typeof local !== 'object') return cloud;

  try {
    const merged = JSON.parse(JSON.stringify(cloud));

    // 1. Preserve Profile & Exam Date
    merged.profile = merged.profile || local.profile;
    merged.examDate = merged.examDate || local.examDate;

    // 2. Merge Daily Logs (Keep logs & highest hours from both devices)
    merged.dailyLogs = merged.dailyLogs || {};
    if (local.dailyLogs) {
      for (const dateKey in local.dailyLogs) {
        const localLog = local.dailyLogs[dateKey];
        if (!localLog) continue;

        if (!merged.dailyLogs[dateKey]) {
          merged.dailyLogs[dateKey] = localLog;
        } else {
          const cloudLog = merged.dailyLogs[dateKey];
          cloudLog.hours = Math.max(Number(cloudLog.hours) || 0, Number(localLog.hours) || 0);
          cloudLog.questions = Math.max(Number(cloudLog.questions) || 0, Number(localLog.questions) || 0);
          cloudLog.completed = Boolean(cloudLog.completed || localLog.completed);

          cloudLog.tasks = Array.isArray(cloudLog.tasks) ? cloudLog.tasks : [];
          const taskIds = new Set(cloudLog.tasks.map(t => t.id));
          (localLog.tasks || []).forEach(t => {
            if (t && t.id && !taskIds.has(t.id)) {
              cloudLog.tasks.push(t);
            }
          });
        }
      }
    ...
    // 3. Merge Chapter Completions
    if (local.chapters && merged.chapters) {
      for (const subj in SYLLABUS) {
        const localList = Array.isArray(local.chapters[subj]) ? local.chapters[subj] : [];
        const cloudList = Array.isArray(merged.chapters[subj]) ? merged.chapters[subj] : [];

        localList.forEach(localCh => {
          if (!localCh) return;
          const cloudCh = cloudList.find(c => c && (c.id === localCh.id || c.name === localCh.name));
          if (cloudCh) {
            cloudCh.lecture = Boolean(cloudCh.lecture || localCh.lecture);
            cloudCh.notes = Boolean(cloudCh.notes || localCh.notes);
            cloudCh.dpp = Boolean(cloudCh.dpp || localCh.dpp);
            cloudCh.pyqMain = Boolean(cloudCh.pyqMain || localCh.pyqMain);
            cloudCh.pyqAdv = Boolean(cloudCh.pyqAdv || localCh.pyqAdv);

            if (Array.isArray(localCh.revisions)) {
              cloudCh.revisions = Array.isArray(cloudCh.revisions) ? cloudCh.revisions : [false,false,false,false,false,false,false];
    ...
    // 4. Merge Mistakes
    if (Array.isArray(local.mistakes)) {
      merged.mistakes = Array.isArray(merged.mistakes) ? merged.mistakes : [];
      const mIds = new Set(merged.mistakes.map(m => m.id));
    ...
    if (local.streak && (Number(local.streak.count) || 0) > (Number(merged.streak.count) || 0)) {
    ...
    merged.updatedAt = new Date(Math.max(localTime, cloudTime)).toISOString();

    return merged;
  } catch (err) {
    console.error("MergeStates error fallback:", err);
    return cloud || local;
  }
}
...
async function pushStateToCloud() {
  ...
  try {
    ...
    updateSyncBadge('🔴 Sync Error: ' + e.message, '#FF4D4D');
  }
}

// 2. Pull State from Cloud (Server-First)
...
async function syncWithCloud() {
  ...
  try {
    ...
    if (cloudData && cloudData.chapters) {
      ...
      updateSyncBadge('🟢 Progress Safely Merged', '#33D6A6');
...
function setupRealtimeSync() {
  ...
  try {
    unsubscribeRealtime = db.collection('users').doc(id).onSnapshot(
      ...
      (doc) => {
        // Ignore optimistic local write echoes
        ...
        if (doc.exists) {
          try {
            ...
            updateSyncBadge('🔴 Sync Parse Error', '#FF4D4D');
          }
        } else {
          updateSyncBadge('🟢 Connected (New User Account)', '#33D6A6');
        }
      },
      (error) => {
...
// 5. WEB BROWSER AUTOMATIC SYNC LISTENERS
if (!isCapacitorApp) {
...
// 7. GLOBAL SAVE OVERRIDE (Updates timestamp on EVERY user action)
async function save() {
  if (!isSyncingFromCloud) {
    state.updatedAt = new Date().toISOString();
  }
  await storageSet('jee-ascend-state', JSON.stringify(state));
  if (!isSyncingFromCloud && typeof pushStateToCloud === 'function') {
    pushStateToCloud();
  }
  syncWidgetData();
}
...
function markLectureDone(c){
  ...
  const base = new Date();
  c.revisionDates = REV_GAPS.map(g => { const d = new Date(base); d.setDate(d.getDate()+g); return localDateStr(d); });
  showCelebrationBurst('Lecture complete', '📚');
  save(); renderAll();
}
...
function markRevisionDone(c, idx){
  if(!c.revisions[idx]){
    ...
    showCelebrationBurst('Revision marked done', '🔁');
  }
  save(); renderAll();
}
...
function showCelebrationToast(message = 'Target complete!', emoji = '🎉'){
  const existing = document.getElementById('celebrationToast');
  ...
  toast.innerHTML = `
    <div class="celebration-badge">${emoji}</div>
  ...
  clearTimeout(window.__celebrationToastTimer);
  window.__celebrationToastTimer = setTimeout(() => {
    toast.classList.remove('show');
    setTimeout(() => toast.remove(), 250);
  }, 1800);
}
function showCelebrationBurst(message = 'Task complete!', emoji = '✨'){
  const existing = document.getElementById('celebrationBurst');
  if(existing) existing.remove();

  const burst = document.createElement('div');
  burst.id = 'celebrationBurst';
  burst.className = 'celebration-burst';
  burst.innerHTML = `
    <div class="burst-core">${emoji}</div>
    <div class="burst-ring burst-r1"></div>
    <div class="burst-ring burst-r2"></div>
    <div class="burst-ring burst-r3"></div>
    <div class="burst-ring burst-r4"></div>
    <div class="burst-ring burst-r5"></div>
    <div class="burst-ring burst-r6"></div>
    <div class="burst-msg">${message}</div>
  `;

  document.body.appendChild(burst);
  requestAnimationFrame(() => burst.classList.add('show'));

  clearTimeout(window.__celebrationBurstTimer);
  window.__celebrationBurstTimer = setTimeout(() => {
    burst.classList.remove('show');
    setTimeout(() => burst.remove(), 300);
  }, 900);

  showCelebrationToast(message, emoji);
}
function toggleCustomTask(id){
  const log = getTodayLog();
  const t = log.tasks.find(x=>x.id===id); t.done = !t.done;
  if(t.done) showCelebrationToast('Target checked off');
  save(); renderDashboard();
}
...
function addTaskForDate(dateStr){
  ...
  input.value = '';
  // Keep the current date selected so it doesn't rebound to Today
  state.ui = state.ui || {};
  state.ui.plannerSelectedDate = dateStr;
  save(); 
  renderPlanner();
  if(dateStr === todayStr()) renderDashboard();
  // Restore focus to the input for easy multi-task entry
  const newInput = document.getElementById('weekTaskInput_'+dateStr);
...
function toggleTaskForDate(dateStr, id){
  ...
  if(t && t.done) showCelebrationToast('Target completed');
  state.ui = state.ui || {};
  state.ui.plannerSelectedDate = dateStr;
  save(); 
  renderPlanner();
...
function openPlannerDay(dateStr){
  state.ui = state.ui || {};
  state.ui.plannerSelectedDate = dateStr;
  save(); renderPlanner();
}
function weekPlannerHTML(){
  ...
  for(let i=0;i<7;i++){
    ...
    const selected = state.ui && state.ui.plannerSelectedDate === dateStr;
    const doneCount = log.tasks.filter(t=>t.done).length;
    const dueChapters = all.map(c=>({c,st:computeRevisionStatus(c)})).filter(x=>x.st && (x.st.overdue || x.st.dueToday));
    const detailHtml = selected ? `<div class="planner-date-details">
      <div class="detail-row"><span>Tasks</span><strong>${log.tasks.length}</strong></div>
      <div class="detail-row"><span>Done</span><strong>${doneCount}</strong></div>
      <div class="detail-row"><span>Due revisions</span><strong>${dueChapters.length}</strong></div>
    </div>` : '';
    html += `<div class="day-card ${i===0?'is-today':''} ${selected?'is-selected':''}" onclick="openPlannerDay('${dateStr}')">
      <div class="day-label">${label}</div>
      <div class="day-date" onclick="event.stopPropagation(); openPlannerDay('${dateStr}')">${dateLabel}</div>
      ${detailHtml}
      <div class="task-add-row">
        <input id="weekTaskInput_${dateStr}" placeholder="Add a task" onclick="event.stopPropagation()" onkeydown="if(event.key==='Enter'){ event.stopPropagation(); addTaskForDate('${dateStr}'); }">
        <button onclick="event.stopPropagation(); addTaskForDate('${dateStr}')">+</button>
      </div>
      ${log.tasks.length ? log.tasks.map(t=>`
        <div class="task-item ${t.done?'on':''}">
          <div class="box ${t.done?'on':''}" onclick="event.stopPropagation(); toggleTaskForDate('${dateStr}','${t.id}')">${t.done?icon('check'):''}</div>
          <div class="txt">${t.text}</div>
          <button class="del" onclick="event.stopPropagation(); deleteTaskForDate('${dateStr}','${t.id}')">&times;</button>
...
function rewardsSectionHTML(log){
  ...
  <div class="rewards-hero">
    ...
    <div style="text-align:right"><div class="pagesub">Today's potential</div><div style="font-family:var(--mono);font-size:calc(20px * var(--font-scale));color:var(--green)">+${potential}</div><div class="pagesub" style="font-size:calc(10.5px * var(--font-scale))">credited when you close out the day</div></div>
  </div>
  <div class="card">
    ${catalogHTML}
...
function renderDashboard(){
  ...
  let html = `<div class="dashboard-hero">
    <div class="greet-card dashboard-greet-card">
      <div class="dashboard-hero-main">
        <div class="dashboard-hero-mark">✦</div>
        <div>
          <div class="greet-name">Hi ${firstName} 👋</div>
          <div class="greet-quote">"${quoteOfDay()}"</div>
        </div>
      </div>
      <div class="dashboard-hero-badges">
        <div class="dashboard-badge">
          <div class="dashboard-badge-label">Streak</div>
          <div class="dashboard-badge-value">${state.streak.count} 🔥</div>
        </div>
        <div class="dashboard-badge">
          <div class="dashboard-badge-label">Mission progress</div>
          <div class="dashboard-badge-value">${progress}%</div>
        </div>
      </div>
    </div>
  </div>`;
  ...
  html += `<div class="mission-card">
      ...
      <div style="display:flex;align-items:center;gap:16px">
        <div class="mission-progress-wrap">
          ...
          <div class="mission-progress-copy">
            <div class="mission-progress-status">${progress >= 100 ? 'Complete' : progress >= 70 ? 'On track' : 'Building momentum'}</div>
            <div class="mission-progress-txt">${progress}% today</div>
          </div>
        </div>
    ...
    <div style="margin-top:16px">
      <div style="font-size:calc(10.5px * var(--font-scale));color:var(--text-3);text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px">Last 30 days</div>
  ...
  for(const s in SYLLABUS){
    html += `<div class="card" style="display:flex;align-items:center;gap:14px">
      ...
      <div><div style="font-weight:600;font-size:calc(14px * var(--font-scale))">${s}</div><div class="sub" style="color:var(--text-3);font-size:calc(11.5px * var(--font-scale))">${state.chapters[s].filter(c=>c.lecture).length}/${state.chapters[s].length} lectures done</div></div>
    </div>`;
  }
  html += `</div>`;
...
function toggleField(chId, subject, field){
  ...
  c[field] = !c[field];
  if(c[field] && !prev){
    const labels = { lecture:'Lecture complete', notes:'Notes completed', dpp:'DPP marked done', pyqMain:'PYQ Main completed', pyqAdv:'PYQ Advanced completed' };
    const icons = { lecture:'📚', notes:'📝', dpp:'🧠', pyqMain:'📘', pyqAdv:'🏆' };
    showCelebrationBurst(labels[field] || 'Task complete', icons[field] || '✨');
  }
  save(); renderSubjects();
}
...
function renderSubjects(){
  ...
  tabs += `</div>`;
  tabs += `<div class="subject-quick-card">
    <div class="subject-quick-copy">
      <div class="subject-quick-title">Add a custom chapter</div>
      <div class="subject-quick-sub">Create a personal chapter card for this subject and keep it in your study flow.</div>
    </div>
    <div class="add-chapter-row">
      <input id="newChapterInput" placeholder="Type a chapter name..." onkeydown="if(event.key==='Enter')addCustomChapter()">
  ...
  (state.chapters[currentSubject]||[]).forEach(c=>{
    ...
    list += `<div class="chapter" id="ch-${c.id}">
      ...
      <div class="chapter-body">
        ...
        <div style="font-size:calc(10.5px * var(--font-scale));color:var(--text-3);text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px">Spaced revision (day 1 / 3 / 7 / 15 / 30 / 60 / 120)</div>
        <div class="rev-track">
...
function renderMistakes(){
  ...
  state.mistakes.forEach(m=>{
    ...
    html += `<div class="mistake-item ${m.fixed?'fixed':''}">
      <div class="body">
        <div class="top"><span class="pill ${pillClass}">${pillLabel}</span><span style="font-size:calc(12px * var(--font-scale));color:var(--text-3)">${m.subject} · ${m.chapter}</span></div>
...
const FOCUS_PRESETS = {
  deep: { label: 'Deep Work', work: 45, break: 10, accent: 'var(--accent)', hint: 'High-concentration study block' },
  sprint: { label: 'Sprint', work: 25, break: 5, accent: 'var(--amber)', hint: 'Quick burst for revision pushes' },
  calm: { label: 'Calm Focus', work: 50, break: 15, accent: '#33D6A6', hint: 'Slower, steady learning pace' },
  creative: { label: 'Creative', work: 35, break: 8, accent: '#A674FF', hint: 'Idea-heavy, flexible problem solving' }
};
let focusTimer = { mode:'focus', remainingSec:25*60, running:false, intervalId:null, initialized:false, preset:'deep' };
...
function playBeep(){
  try{
    ...
    const ctx = window.__focusAudioCtx || new AudioCtor();
    window.__focusAudioCtx = ctx;
    ...
    const pattern = [
      { freq: 880, type: 'square', gain: 0.55, duration: 0.95, offset: 0 },
      { freq: 1320, type: 'sawtooth', gain: 0.58, duration: 0.95, offset: 0.18 },
      { freq: 1760, type: 'square', gain: 0.62, duration: 1.10, offset: 0.36 },
      { freq: 2200, type: 'sine', gain: 0.48, duration: 1.25, offset: 0.58 }
    ];
    ...
    pattern.forEach((note) => {
      ...
      oscillator.frequency.setValueAtTime(note.freq, startAt + note.offset);
      gain.gain.setValueAtTime(0.0001, startAt + note.offset);
      gain.gain.exponentialRampToValueAtTime(note.gain, startAt + note.offset + 0.03);
    ...
    if(typeof navigator !== 'undefined' && navigator.vibrate){
      navigator.vibrate([260, 120, 260, 120, 260]);
    }
  }catch(e){}
}
function focusTotalSec(){ return (focusTimer.mode==='focus' ? state.settings.focusWorkMin : state.settings.focusBreakMin) * 60; }
function setFocusPreset(name){
  const preset = FOCUS_PRESETS[name];
  if(!preset) return;
  focusTimer.preset = name;
  state.settings.focusWorkMin = preset.work;
  state.settings.focusBreakMin = preset.break;
...
function startFocusTimer(){
    ...
    if(typeof notifyFocusStart === 'function') notifyFocusStart();
...
function skipBreakTimer(){
  if(focusTimer.mode !== 'break') return;
...
function completeFocusSession() {
    ...
    if (focusTimer.mode === "focus") {
        ...
        save();
        if(typeof notifyFocusComplete === 'function') notifyFocusComplete();

        focusTimer.mode = "break";
        focusTimer.remainingSec = state.settings.focusBreakMin * 60;

    } else {
      if(typeof notifyBreakComplete === 'function') notifyBreakComplete();
...
function renderFocus(){
  ...
  const activePreset = FOCUS_PRESETS[focusTimer.preset] || FOCUS_PRESETS.deep;
  const log = getTodayLog();
  el.innerHTML = `
  <div class="focus-wrap ${focusTimer.preset}">
    <div class="focus-mode-label ${onBreak?'on-break':''}">${onBreak ? 'Break' : activePreset.label}</div>
    <div class="focus-preset-row">
      ${Object.entries(FOCUS_PRESETS).map(([key, preset]) => `
        <button class="focus-preset-btn ${focusTimer.preset===key?'active':''}" onclick="setFocusPreset('${key}')">${preset.label}</button>
      `).join('')}
    </div>
    <div class="focus-preset-hint">${activePreset.hint}</div>
    ...
    <div class="focus-controls">
      ...
      ${onBreak ? '<button class="focus-btn" onclick="skipBreakTimer()">Skip break</button>' : '<button class="focus-btn" onclick="resetFocusTimer()">Reset</button>'}
      ${onBreak ? '<button class="focus-btn" onclick="resetFocusTimer()">Reset</button>' : ''}
    </div>
...
function renderSettings(){
  ...
  <div class="settings-hero">
    <div class="settings-hero-copy">
      ...
      <div class="settings-hero-title">Tune your JEE system</div>
      <div class="settings-hero-sub">Daily targets, rewards, backup, and profile controls in one polished place.</div>
    </div>
    <div class="settings-hero-metrics">
      <div class="metric-pill"><span>Hours target</span><strong>${state.settings.hoursTarget || 0}</strong></div>
      <div class="metric-pill"><span>Questions target</span><strong>${state.settings.questionsTarget || 0}</strong></div>
      <div class="metric-pill"><span>Reward / hour</span><strong>${state.settings.rewardPointsPerHour || 0} pts</strong></div>
    </div>
  </div>
  ...
  <div class="settings-block">
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">
        <button class="add-chapter-row button" style="background:var(--accent);color:#fff;border:none;padding:9px 18px;border-radius:8px;font-size:calc(13px * var(--font-scale));cursor:pointer" onclick="exportData()">Export backup (.json)</button>
        <button style="background:var(--panel-2);border:1px solid var(--line);color:var(--text);padding:9px 18px;border-radius:8px;font-size:calc(13px * var(--font-scale));cursor:pointer" onclick="document.getElementById('importFileInput').click()">Import backup</button>
...
// ---------- NAV + INIT ----------
let bottomNavScrollIdleTimer = null;
let bottomNavScrollLastY = 0;
const TITLES = {
...
(async function init(){
  ...
  setTimeout(()=>{setTimeout(async () => {
    ...
    if (typeof requestNotificationPermission === "function") {
        ...
        if(typeof scheduleStreakReminder === 'function') scheduleStreakReminder();
  ...
  document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) checkDayRollover(); else pushStateToCloud(); });
  window.addEventListener('beforeunload', ()=>{ pushStateToCloud(); });
  registerBottomNavScrollBehavior();
})();
...
function handleBottomNavScroll() {
  ...
  const scrollingDown = currentY > bottomNavScrollLastY;

  if (scrollingDown && currentY > 12) {
    nav.classList.add('nav-hidden');
  }

  bottomNavScrollLastY = currentY;

  window.clearTimeout(bottomNavScrollIdleTimer);
  bottomNavScrollIdleTimer = window.setTimeout(() => {
    nav.classList.remove('nav-hidden');
  }, 160);
}

function registerBottomNavScrollBehavior() {
  bottomNavScrollLastY = window.scrollY || document.documentElement.scrollTop || document.body.scrollTop || 0;
...
async function syncWidgetData(){
  ...
  try{
    const { WidgetBridge } = Capacitor.Plugins;
    if(WidgetBridge) await WidgetBridge.setStreak({ count: state.streak.count });
